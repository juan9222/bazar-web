        ...
        <Col md={ 8 } className='block__right'>
          <div className="cc__content">
            <div className="cc__content--form">
              ...
              <Modal title="" continueText={ isBuyer(authenticatedUser) ? 'Continue' : 'Create product now' } cancelText='Create product later' width='590px' closed={ !showCongratulationsModal } showCloseIcon={ false } onClose={ () => setShowConfirmationModal(false) } onContinue={ isBuyer(authenticatedUser) ? onCreateProductLater : onCreateProduct } onCancel={ onCreateProductLater } cancelHidden={ isBuyer(authenticatedUser) } loading={ loading }>
                < div className="verticalSpaceS"></div>
                <h3 className='textPrimary300 textModalTitle dFlex aICenter'> <AiFillCheckCircle className='textSuccess200' />             <div className="horizontalSpaceS"></div>
                  Congratulations</h3>
                <div className="verticalSpaceL"></div>
                <p className='textModalDesc'>Your profile is almost ready, it just needs a verification by our team, it won't take long! { isBuyer(authenticatedUser) && (<b>You can create your product list, while waiting for our validation.</b>) }</p>
              </Modal>
            </div>
          </div>
        </Col>
