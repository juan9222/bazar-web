...
export const productDetailFormValidator = (maxQuantity?: number) => Yup.object().shape<TProductDetailFormValidatorShapeKeys>({
  ...
  quantity: Yup.number().min(1, "Must be more than 0").max(maxQuantity ?? 1000000000, "Available quantity exceeded.").typeError("This field is required").required("This field is required"),
});